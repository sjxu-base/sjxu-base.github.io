...
Pod 是 Kubernetes 抽象出来的，表示一组一个或多个应用程序容器（如 Docker，containerd），以及这些容器的一些共享资源。这些资源包括:

- 共享存储
- 网络，作为唯一的集群 IP 地址
- 有关每个容器如何运行的信息，例如容器映像版本或要使用的特定端口

Pod 是 Kubernetes 平台上的**原子单元**。当我们在 Kubernetes 上创建 Deployment 时，该 Deployment 会在其中创建包含容器的 Pod （而不是直接创建容器）。每个 Pod 都与调度它的工作节点绑定，并保持在那里直到终止（根据重启策略）或删除。 如果工作节点发生故障，则会在群集中的其他可用工作节点上调度相同的 Pod。

一个 pod 总是运行在 **Nodes**。工作节点是 Kubernetes 中的参与计算的机器，可以是虚拟机或物理计算机，具体取决于集群。每个工作节点由主节点管理。工作节点可以有多个 pod ，Kubernetes 主节点会自动处理在群集中的工作节点上调度 pod 。 主节点的自动调度考量了每个工作节点上的可用资源。

每个 Kubernetes 工作节点至少包含三个组件:

- kubelet，负责 Kubernetes 主节点和工作节点之间通信的过程; 它管理 Pod 和机器上运行的容器。
- cri，container runtime interface，容器运行时（如 Docker，containerd）负责从仓库中提取容器镜像，解压缩容器，并运行应用程序
- cni，container network interface，容器
...
Kubernetes 中的服务(Service)是一种抽象概念，它定义了 Pod 的集合和访问 Pod 的方法。

Service 使从属 Pod 之间的松耦合成为可能。 Service 用 YAML 来定义. Service 下的一组 Pod 通常由 *LabelSelector* 来标记。

尽管每个 Pod 都有一个唯一的 IP 地址，但是如果没有 Service ，这些 IP 无法暴露在群集外部。

Service 允许应用程序接收流量，并用不同方式转发，具体的方式由 ServiceSpec 标记 `type` 决定。

### ClusterIP Service

默认类型 Service，用于在集群的内部 IP 上公开 Service 。这种类型使得 Service 只能从集群内访问。
...
通过将带有该 Service 名称的 CNAME 记录注册到集群的 DNS，使用由 Service 配置中的 `externalName` 指定的名称向集群内公开集群外部服务。

不使用代理。这种类型需要 `kube-dns` 的 v1.7 以上版本方可支持。
