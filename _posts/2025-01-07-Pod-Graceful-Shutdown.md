...
1. Pending
   Pod 已提交但尚未调度到 Node，或镜像正在拉取。
2. Running
   至少一个容器成功创建并在运行，Pod 处于正常服务阶段。
3. Succeeded / Failed
   容器正常退出（退出码 0 为 Succeeded），或运行失败（退出码非 0 为 Failed）。
4. Unknown
   Node 无法访问，状态不可知。
...
- PreStop 必定在 SIGTERM 之后、容器退出之前调用
- 若 PreStop 本身阻塞，则会占满 terminationGracePeriodSeconds
- 必须设计得可快速执行，否则会导致超时被 SIGKILL
...
用户删除 Pod / 控制器重建 / 节点驱逐
          │
          ▼
kubelet 将 Pod 状态改为 Terminating
          │
          ▼
从 Service Endpoint 中移除 Pod
（停止接收新请求）
          │
          ▼
发送 SIGTERM 给容器内主进程
          │
          ▼
执行 PreStop Hook（同步执行）
（可能延迟容器退出，但不会阻止 SIGTERM）
          │
          ▼
等待 terminationGracePeriodSeconds（默认 30s）
          │
          ▼
若容器还未退出：发送 SIGKILL 强制杀死
          │
          ▼
容器退出 → CNI 清理网络 → 卷卸载 → Pod 回收

常见误区：PreStop 和 SIGTERM 谁先？
实际上：SIGTERM 先 → 然后执行 PreStop。
PreStop 不是一个 "pre-SIGTERM"，而是 "pre-termination"。

## 案例：处理 Orphan Pod 问题（涉及 Ceph PVC）

问题来源：https://github.com/kubernetes/kubernetes/issues/60987
...
- 卷未卸载成功
- Pod 对象被删除，但文件系统仍被节点占用
- 最终形成 orphaned pod directory / orphaned volume
...
- Kubernetes 官方文档: Container Lifecycle Hooks
- kubelet graceful node shutdown
- Issue #60987 Orphaned pod directories due to Ceph volume
