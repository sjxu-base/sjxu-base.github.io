...
- `systemd` 系统管理服务
  ...
  `systemd` 本身是一个**初始化系统**（init system），用于取代传统的 `SysVinit` 和 `Upstart`。
  
  启动时由内核调用，负责**初始化系统、挂载文件系统、启动服务**等。同时还包含**定时器、登录会话管理、日志管理**等子模块。

- `systemctl` 系统管理工具
  
  操控 `systemd` 的主力工具，支持启动、停止、查看、配置服务等操作。

- `journalctl` 诊断工具
  
  用于查看 systemd 的日志系统**，是 `systemd` 内建的日志管理工具，用来替代传统的 `/var/log/messages` 或 `rsyslog`
